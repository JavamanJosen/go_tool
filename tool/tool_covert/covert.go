...
func DecBin(n int64) string {
	if n < 0 {
		log.Info("Decimal to binary error: the argument must be greater than zero.")
		return ""
	}
	if n == 0 {
		return "0"
...
func DecOct(d int64) int64 {
	...
	if d < 0 {
		log.Info("Decimal to octal error: the argument must be greater than zero.")
		return -1
	}
	s := ""
	for q := d; q > 0; q = q / 8 {
	...
	if err != nil {
		log.Info("Decimal to octal error:", err.Error())
		return -1
	}
	return int64(n)
}
...
func DecHex(n int64) string {
	if n < 0 {
		log.Info("Decimal to hexadecimal error: the argument must be greater than zero.")
		return ""
	}
	if n == 0 {
		return "0"
...
func BinDec(b string) (n int64) {
	...
	for i = 0; i < l; i++ {
		...
		if err != nil {
			log.Info("Binary to decimal error:", err.Error())
			return -1
		}
...
func OctDec(o int64) (n int64) {
	...
	for i = 0; i < l; i++ {
		f, err := strconv.ParseFloat(s[i], 10)
		if err != nil {
			log.Info("Octal to decimal error:", err.Error())
			return -1
		}
...
func HexDec(h string) (n int64) {
	...
	for i = 0; i < l; i++ {
		...
		if err != nil {
			log.Info("Hexadecimal to decimal error:", err.Error())
			return -1
		}
